...
from api.response.get_your_message import ResponseYourMessageDtoSchema
...
class MessagesEndpoint(BaseEndpoint):
    ...
    async def method_post(
        ...
        try:
            message_queries.create_message(session=session, message=request_model, sender=token['user_auth_login'])
    ...
    async def method_get(
        ...
        db_messages = message_queries.get_message(session=session, login=token['user_auth_login'])

        response_body = {
            'received_messages': ResponseGetMessagesDtoSchema(many=True).dump(db_messages),
            'sent_messages': ResponseYourMessageDtoSchema(many=True).dump(db_messages)
        }
